...
public class Console extends StructureObject{
	...
	public Console(Engine engine, HumanClient player){
		super(engine);
		...
		public boolean execute(String[] readReadyCommand) throws CommandExecutionException{
			...
			case "using":
				if(readReadyCommand[1].equals("system_old")){
					out("Changed the command execution system to default");
					out("The usage of this command system is not recommended as it "
							+ "is no longer supported and is likely to cause an error");
					out("type 'using system_complex' to return");
					execution = 0;
				}
				else if(readReadyCommand[1].equals("system_complex")){
					out("Changed the command execution system to complex");
					execution = 1;
				}
