...
public class ItemList{
	...
	public void addItem(int slot, Item item){
		if(items[slot - 1] == null)	{
			...
			((HumanClient)((Hero)owner).owner).out("Slot number: " + slot + " is full. Attemting to add to first free slot");
			if(addIntoNextFirstSlot(item) == 0)((HumanClient)((Hero)owner).owner).out("Unable to add item to inventory. Inventory is full");
			return;
		}
		switch(type){
		case STASH:
